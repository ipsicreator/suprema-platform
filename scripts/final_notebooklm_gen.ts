...
import { model } from '../lib/ai/gemini'; // Use existing working config
...
async function main() {
  ...
  console.log(`Extracting for ${schoolDirs.length} schools using shared gemini config...`);
  let allData: any[] = [];
  ...
  if (allData.length > 0) {
    ...
    console.log("FAILED: Check environment variables and API access.");
  }
}
